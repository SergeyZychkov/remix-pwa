...
self.addEventListener("fetch", function(event) {
  if (dynamicUrlArray.some(function(element) {
...
self.addEventListener("push", (event) => {
  var data2 = { title: "New!", content: "Something new happened!", openUrl: "/" };
  if (event.data) {
    data2 = JSON.parse(event.data.text());
  }
  const options = {
    body: data2.content,
    icon: "/logo192.png",
  ...
  event.waitUntil(
    self.registration.showNotification(data2.title, options)
  );
});
